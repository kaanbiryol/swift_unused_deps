...
public enum BatchAnalyzer {
    ...
    private static func analyzeTarget(
        ...
        let sourceFileUsage = preserveConformanceProviderImports(
            in: filterReferencedModules(
                rawSourceFileUsage,
                for: metadata,
    ...
    private struct ConformanceDeclaration {
        let typeName: String
        let protocolName: String
    }

    private static func preserveConformanceProviderImports(
    ...
    private static func conformanceDeclarations(in source: String) -> [ConformanceDeclaration] {
        let pattern = #"extension\s+([A-Za-z_][A-Za-z0-9_\.]*)\s*:\s*([^\{\n]+)"#
        guard let regex = try? NSRegularExpression(pattern: pattern) else {
            return []
        }
        let range = NSRange(source.startIndex..<source.endIndex, in: source)
        return regex.matches(in: source, range: range).flatMap { match -> [ConformanceDeclaration] in
            guard
                let typeRange = Range(match.range(at: 1), in: source),
                let protocolsRange = Range(match.range(at: 2), in: source)
            else {
                return []
            }
            let typeName = String(source[typeRange])
            return source[protocolsRange]
                .split(separator: ",")
                .compactMap { rawProtocol -> ConformanceDeclaration? in
                    guard let protocolName = normalizedConformanceProtocolName(String(rawProtocol)) else {
                        return nil
                    }
    ...
    private static func normalizedConformanceProtocolName(_ rawValue: String) -> String? {
        let tokens = rawValue
