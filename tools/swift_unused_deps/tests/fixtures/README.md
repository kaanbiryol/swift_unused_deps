...
| `//cases/Targets/UnusedTransitiveImport:UnusedTransitiveImport` | Reports `unused_import` for directly imported transitive `TransitiveDep` and `unused_dep` for its wrapper dep | Bats asserts the report and fix flow remove the import and wrapper BUILD dep without adding the transitive dep |
...
| `//cases/Tests/TestableImports/...` | Distinguishes required, public-only, and unused `@testable` imports | Bats verifies required internal access stays clean while safe candidates are low-confidence and report-only |
