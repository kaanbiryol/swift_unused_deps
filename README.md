...
## Macro-Generated Swift Libraries

`swift_unused_deps` detects generated `swift_library` targets because the aspect
runs on Bazel's analyzed graph. BUILD fixes need to know the editable macro call
and macro attribute that own the deps.

For handwritten targets and macro primary targets, no changes are needed. Fixes
default to the analyzed target and the `deps` attribute.

For secondary targets generated by a macro, stamp these tags on the generated
`swift_library`:
...
A finding on `//pkg:FooTestsLib` can then edit the macro call:

```text
buildozer 'remove test_deps //x:Bar' //pkg:Foo
```

See [Macro-Generated Targets](docs/macro-generated-targets.md) for the full tag
contract.
